...
import { customUpgradeWorldFixture, World } from './utils/TestWorld';
import {
  ARTIFACT_PLANET_1,
  LVL1_ASTEROID_2,
...
describe('DarkForestCustomUpgrade', function () {
  ...
  it('should allow upgrade on upgradeable planet types', async function () {
    this.timeout(0);
    await world.user1Core.initializePlayer(...makeInitArgs(SPAWN_PLANET_1));
    ...
    if (upgradeablePlanets[PlanetType.ASTEROID]) {
      await expect(world.user1Core.upgradePlanet(LVL1_ASTEROID_2.id, 0)).to.emit(world.contract, 'PlanetUpgraded')
        .withArgs(world.user1.address, LVL1_ASTEROID_2.id, BN.from(0), BN.from(1));
    }

    if (upgradeablePlanets[PlanetType.RIP]) {
      await expect(world.user1Core.upgradePlanet(LVL3_SPACETIME_1.id, 0)).to.emit(world.contract, 'PlanetUpgraded')
        .withArgs(world.user1.address, LVL3_SPACETIME_1.id, BN.from(3), BN.from(4));
    }

    if (upgradeablePlanets[PlanetType.FOUNDRY]) {
      await expect(world.user1Core.upgradePlanet(ARTIFACT_PLANET_1.id, 0)).to.emit(world.contract, 'PlanetUpgraded')
        .withArgs(world.user1.address, ARTIFACT_PLANET_1.id, BN.from(0), BN.from(1));
    }

    if (upgradeablePlanets[PlanetType.QUASAR]) {
      await expect(world.user1Core.upgradePlanet(LVL1_QUASAR.id, 0)).to.emit(world.contract, 'PlanetUpgraded')
        .withArgs(world.user1.address, LVL1_QUASAR.id, BN.from(1), BN.from(2));
    }
  });

  it('should reject upgrade on non-upgradeable planet types', async function () {
    this.timeout(0);
    await world.user1Core.initializePlayer(...makeInitArgs(SPAWN_PLANET_1));
  ...
  it('should recognize upgradeable planets config', async function (){
    const gameConstants = await world.contract.getGameConstants();
